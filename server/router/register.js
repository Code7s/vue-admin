...
router.post('/register', async (req, res) => {
  ...
  if (!emailReg.test(email)) {
    ...
    res.send({ tip: '至少输入6个字符，最多不超过16个字符,允许小数点和下划线', status: 2 })
    return
  } else if (password !== res_password) {
    res.send({ tip: '两次输入的密码不同,请再次输入！', status: 3 })
    return
  }
  ...
  try {
    ...
    if (findResult) {
      res.send({ tip: '该邮箱已经被注册！', status: 4 })
    } else {
      // 确保数据存入数据库再响应注册成功
      await userModel.create({ email, password })
      res.send({ tip: '注册成功！请返回登录', status: 0 })
    }
  } catch (err) {
    console.log(err)
    res.send('出问题了,请稍后再试！')
